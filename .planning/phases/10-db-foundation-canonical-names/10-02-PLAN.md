...
must_haves:
  truths:
    ...
    - "Beverages receive is_beverage=true and enrichment_status='skipped'; food items receive enrichment_status='pending'"
...
6. **Process results**: For each result from LLM output:
   ...
   - Determine `enrichment_status` based on confidence and beverage flag:
     - `is_beverage = true` → `enrichment_status = 'skipped'` (per KNOW-04: beverages deprioritized)
...
8. Beverages get `is_beverage=true` and `enrichment_status='skipped'`
</verification>
...
- Beverages are flagged and deprioritized for enrichment (KNOW-04)
